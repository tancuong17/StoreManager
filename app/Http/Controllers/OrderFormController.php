...
class OrderFormController extends Controller
{
    ...
    public function gets(Request $request)
    {
      try {
        if($request->keyword){
          ...
          $orders = OrderForm::select('order_forms.id', 'order_forms.code', 'order_forms.table_number', 'order_forms.note', 'order_forms.updated_at', 'users.name as updater')->join('users', 'order_forms.updater', '=', 'users.id')->where("status", $request->status)->orderBy('order_forms.updated_at', 'DESC')->offset(((int)$request->page - 1) * 12)->limit(12)->get();
        }
        if(count($orders) != 0)
    ...
    {
      try {
        $orderQuantity = OrderForm::where("status", 0)->whereDate("created_at", ">=", $request->dateFrom)->whereDate("created_at", "<=", $request->dateTo)->count();
        $billQuantity = OrderForm::where("status", 1)->whereDate("created_at", ">=", $request->dateFrom)->whereDate("created_at", "<=", $request->dateTo)->count();
        $productQuantity = OrderForm::select("detail_order_forms.id", "prices.price")->join('detail_order_forms', 'detail_order_forms.order_form', '=', 'order_forms.id')->join('products', 'products.id', '=', 'detail_order_forms.product')->join('prices', 'prices.product', '=', 'products.id')->where("order_forms.status", 1)->whereDate("order_forms.updated_at", ">=", $request->dateFrom)->whereDate("order_forms.updated_at", "<=", $request->dateTo)->whereRaw('prices.created_at = prices.updated_at')->where("order_forms.status", 1)->orWhereDate("order_forms.updated_at", "=", $request->date)->whereRaw('prices.created_at <= order_forms.updated_at')->whereRaw('prices.updated_at >= order_forms.updated_at')->sum("detail_order_forms.quantity");
